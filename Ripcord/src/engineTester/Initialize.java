...
public final class Initialize {
	...
	public static Billboard loadBillboard(Vector3f position, Vector3f rotation, Vector2f scale, String texture){

		ModelTexture gunTexture = new ModelTexture(loader.loadTexture(MyPaths.makeTexturePath("Terrain/"+texture)));
		ModelData gunData = OBJLoader.loadOBJ(MyPaths.makeOBJPath("Static/quad"));
		gunTexture.setHasTransparency(true);
		gunTexture.setUseFakeLighting(true);
		RawModel RawGun = loader.loadToVAO(gunData.getVertices(), gunData.getTextureCoords(), gunData.getNormals(), gunData.getIndices());
		TexturedModel texturedGun = new TexturedModel(RawGun,gunTexture,1);
		
		return new Billboard(position, scale, rotation, texturedGun);
	}
	
	protected static Gun loadGun(short weaponID){
	...
	public static MultiModeledEntity loadMultiModeledEntity(String name, Vector3f position, float rotX, float rotY, float rotZ, Vector3f scale,
		...
		MultiModeledEntity barn = new MultiModeledEntity(masterList, position, rotX, rotY, rotZ, scale, name, b_model, number_of_parts, true);
	...
	public static MultiModeledEntity newMultiModeledEntity(String name, Vector3f position, int parts, boolean b_model){
		...
		MultiModeledEntity barn = loadMultiModeledEntity(name, position, 0, 0, 0, new Vector3f(1,1,1), 1, false, 1, 1, false, 425, parts, b_model);
		
		return barn;
	}
	
	public static AnimatedEntity newAnimatedEntity (String name, int num, Vector3f position, int timeBetween){
		
		ModelData gunData = OBJLoader.loadOBJ(MyPaths.makeOBJPath(name));
		ModelData gunData2 = OBJLoader.loadOBJ(MyPaths.makeOBJPath(name + "C"));
		RawModel RawGun = loader.loadToVAO(gunData.getVertices(), gunData2.getVertices(), gunData.getTextureCoords(), gunData.getNormals(), gunData2.getNormals(),
				gunData.getIndices(), gunData2.getIndices());
		ModelTexture gunTexture = new ModelTexture(loader.loadTexture(MyPaths.makeTexturePath("Mobs/"+name)));
		TexturedModel texturedGun = new TexturedModel(RawGun,gunTexture,1);
		
		gunTexture.setShine_damper(5);
		gunTexture.setReflectivity(0.3f);
		
		AnimatedEntity barn = new AnimatedEntity(texturedGun, position, 0, 40, 0, new Vector3f(2,2,2), 1, name, false, timeBetween);
		barn.setID(num);
