...
public class MasterRenderer {
	...
	private Map<TexturedModel, List<Billboard>> billboards = new HashMap <TexturedModel, List<Billboard>>();
	...
	public void render(List<Light> light, Camera cam, boolean wireframe, Mode mode){
		...
		if (waters.size() > 0){
			...
			final float distance = 2*(cam.getPosition().y - Statics.SEA_LEVEL);
	...
	public void renderScene(List<Light> light, Camera cam, boolean wireframe, Vector4f clipPlane, boolean skybox, Mode mode){
		...
		renderer.render(billboards, wireframe, cam.getPosition());
		shader.stop();
	}
	...
	public void processBillboard(Billboard billboard){
		...
		TexturedModel model = billboard.getModel();
			
		List<Billboard> batch = billboards.get(model);
			
		if (batch!=null){
			batch.add(billboard);
		}else{
			List<Billboard> newBatch = new ArrayList<Billboard>();
			newBatch.add(billboard);
			billboards.put(model, newBatch);
		}
