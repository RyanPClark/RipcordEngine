...
public final class MapLoader {
	...
	public static List<MultiModeledEntity> Load(){
		...
		try{
			...
			Cryption.decrypt(new File(mapdata));
			
			  FileInputStream fstream = new FileInputStream(mapdata);
			  ...
			  while ((strLine = br.readLine()) != null)   {
				 ...
				 if(strLine.startsWith("x:")){
					 ...
					 MultiModeledEntity ent = Initialize.loadMultiModeledEntity(currentLine[1], position, rotX, rotY, rotZ, scale, (int)textureAtlasID,
								transparency, shineDamper, reflectivity, fakeLighting, frustumFactor.intValue(), number_of_parts, b_model);
			  ...
			  in.close();
				...
				Cryption.encrypt(new File(mapdata));
