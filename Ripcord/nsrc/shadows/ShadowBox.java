...
public class ShadowBox {
	 ...
	 *            - the in-game camera.
	 */
	protected ShadowBox(Matrix4f lightViewMatrix, Entity camera) {
	...
	protected void update() {
		...
		Position pos = (Position)cam.getComponentByType(CompType.POSITION);
		Vector3f centerNear = Vector3f.add(toNear, pos.getPosition(), null);
		Vector3f centerFar = Vector3f.add(toFar, pos.getPosition(), null);
	...
	private Matrix4f calculateCameraRotationMatrix() {
		...
		rotation.rotate((float) Math.toRadians(-rot.getRotation().y), new Vector3f(0, 1, 0));
		rotation.rotate((float) Math.toRadians(-rot.getRotation().x), new Vector3f(1, 0, 0));
		return rotation;
	}
